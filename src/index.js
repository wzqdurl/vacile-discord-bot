...
async function callCloudflare(messages) {
  const response = await fetch(
    `https://api.cloudflare.com/client/v4/accounts/${process.env.CLOUDFLARE_ACCOUNT_ID}/ai/run/${process.env.CLOUDFLARE_MODEL || '@cf/qwen/qwen3-30b-a3b-fp8'}`,
    {
      ...
      body: JSON.stringify({ messages, max_tokens: 75, temperature: 1 }),
    },
  );
...
async function callGemini(messages) {
  ...
  const response = await fetch(
    `https://generativelanguage.googleapis.com/v1beta/models/${process.env.GEMINI_MODEL || 'gemini-2.5-flash-lite'}:generateContent?key=${process.env.GEMINI_API_KEY}`,
    {
      method: 'POST',
      ...
      body: JSON.stringify({
        ...
        generationConfig: { temperature: 1, maxOutputTokens: 75 },
      }),
    },
  );
  const payload = await response.json();
...
async function generateResponse(messages) {
  const estimatedTokens = estimateInputTokens(messages) + 75;
  const cloudflareNeurons = (estimateInputTokens(messages) * 0.004625) + (75 * 0.030475);
  const providers = [
  ...
  for (const provider of providers) {
    ...
    try {
      if (!await reserveBudget(provider.name, provider.units, provider.limit)) continue;
